...
import React from 'react'; // Keine Verwendung von `useEffect` oder `use client`
import botpressClient from '../bottpress/config';
import { api as publicOpenapi } from '@botpress/api';

async function BotpressTest() {
    try {
        // OpenAPI Client Export
        void publicOpenapi.exportClient('./src/gen/opapi');

        const { bot } = await botpressClient.getBot({ id: process.env.NEXT_PUBLIC_BOTPRESS_BOT_ID! });
        console.log('Bot-Daten erfolgreich abgerufen:', bot);
    } catch (error) {
        console.error('Fehler beim Abrufen der Bot-Daten:', error);
    }

    return <div>Botpress Test wird durchgeführt...</div>;
}
